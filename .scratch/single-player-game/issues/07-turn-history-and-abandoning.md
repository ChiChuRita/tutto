...
- [ ] Each completed Turn is written to a history table indexed by Game
- [ ] A recorded Turn holds the Cards drawn, every Roll's six faces, each set-aside step, each Tutto, and how the Turn ended
- [ ] The Game document holds only live state and does not grow as a Game progresses
- [ ] A Player can abandon a Game in progress
- [ ] An abandoned Game keeps final scores, records no winner, and is marked as abandoned
- [ ] Finished and abandoned Games appear in a list the Player can open
- [ ] Recording history does not change any rules behaviour or existing test
