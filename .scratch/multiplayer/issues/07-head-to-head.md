...
- [x] A signed-in Player sees a stats screen; a guest is told what signing up would give them
- [x] The screen shows the Player's best single Turn across all their Games
- [x] One row per opponent, showing Games shared, wins and losses
- [x] A Game won by more than one Seat counts as neither a win nor a loss for those Seats
- [x] Abandoned Games appear nowhere in the numbers
- [x] Guest Seats are skipped, so a Game against guests shows no opponent rows
- [x] A Game with three or more Seats produces the right record against each opponent
- [x] A Player with no finished Games sees an empty state, not a broken screen
- [x] The derivation is a pure module with its own tests, mirroring the existing Turn-history
