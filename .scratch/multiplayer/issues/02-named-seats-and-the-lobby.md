...
- [ ] A newly created Game is in a lobby, not in play
- [ ] Taking a Seat requires a name and adds you to the Seat list
- [ ] The Seat list shows everyone seated, in join order
- [ ] A name already taken in this Game is refused, with the reason shown
- [ ] The same name in a different Game is allowed
- [ ] The last name used is offered as the default the next time you take a Seat
- [ ] Starting the Game moves it from lobby into play, in join order
- [ ] A Game cannot be started with no Seats
- [ ] A Game with a single Seat can be started and played through to a finish
- [ ] Seat names replace »Platz N« during play and on the result screen
- [ ] The lobby rules are covered by reducer tests alongside the existing ones — no new test
