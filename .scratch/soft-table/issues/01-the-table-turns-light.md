...
- [ ] Every screen — start, lobby, play, result, record — is on the light ground and legible
- [ ] `--color-muted` clears 4.5:1 on every surface; numbers reported per surface
- [ ] Card faces are unchanged, and ink still clears 4.5:1 on all four at corner-index size
- [ ] The dice are still objects and not holes in the page — or `02` is why this ticket stops short
- [ ] Shadows read on a light ground, and are a tint rather than black
- [ ] The play screen does not scroll at 390×844, 375×667 or 375×553 — numbers reported, both regimes
- [ ] No die is smaller and the `1.8 × --die-size` sweep is unchanged
- [ ] Nothing shifts position between phases
- [ ] Colour is tokens in `@theme` and nothing outside it
- [ ] `src/game/turn.ts` and everything under `convex/` are untouched
